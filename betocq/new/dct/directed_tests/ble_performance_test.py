...
Test steps:
  1. Set up a connection with BLE as connection medium.
  2. Transfer file.
...
THROUGHPUT_TARGET = nc_constants.DCT_BLE_MEDIUM_THROUGHPUT_BENCHMARK_MBPS
...
_PAYLOAD_TRANSFER_SIZE_KB = nc_constants.TRANSFER_FILE_SIZE_100KB
_PAYLOAD_TRANSFER_TIMEOUT = nc_constants.BLE_100K_PAYLOAD_TRANSFER_TIMEOUT
_PAYLOAD_TYPE = nc_constants.PayloadType.BYTES
...
class BlePerformanceTest(performance_test_base.PerformanceTestBase):
  ...
  def _setup_android_device(self, ad: android_device.AndroidDevice) -> None:
    nc_utils.setup_android_device_for_nc_tests(
        ad,
        snippet_confs=[self.dct_snippet_config],
        country_code=self.test_runtime.country_code,
        debug_output_dir=self.current_test_info.output_path,
    )
    setup_utils.set_flag_dct_on(ad, self.current_test_info.output_path)
  ...
  def test_ble_performance(self):
    """Test the performance of BLE."""
    # Test Step: Set up a DCT connection for payload transfer.
    ...
    try:
      self.current_test_result.file_transfer_throughput_kbps = (
          active_snippet.transfer_file(
              file_size_kb=_PAYLOAD_TRANSFER_SIZE_KB,
    ...
    logging.info('DCT speed target: %s', THROUGHPUT_TARGET)
