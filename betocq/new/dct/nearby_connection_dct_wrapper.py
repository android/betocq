...
"""Utils for handling DCT Connection rpc."""
...
class NearbyConnectionDctWrapper:
  """Wrapper for DCT Connection Snippet Client Operations."""
  ...
  def __init__(
    ...
    self.password = self._generate_auth_password()
    ...
    self._endpoint_info: bytearray = None

  def start_advertising(self, timeout: datetime.timedelta) -> None:
    """Starts DCT Connection advertising."""
  ...
  def start_discovery(
    ...
    """Starts DCT Connection discovery."""
  ...
  def stop_advertising(self) -> None:
    """Stops DCT Connection advertising."""
  ...
  def stop_discovery(self, enable_target_discovery: bool = False) -> None:
    """Stops DCT Connection discovery."""
  ...
  def accept_connection(self, timeout: datetime.timedelta) -> None:
    ...
    self._discoverer_payload_callback = self.discoverer_nearby.acceptConnection(
        self._advertiser_endpoint_id,
        self._endpoint_info,
    )
  ...
  def disconnect_endpoint(self) -> None:
    """Disconnects DCT Connection endpoint."""
    if self:
      self.discoverer_nearby.disconnectFromDevice(
          self._advertiser_endpoint_id,
          self._endpoint_info,
      )
      self.discoverer.log.info(
      ...
      self.discoverer.log.info('no nearby connection setup yet')
  ...
  def start_nearby_connection(
    ...
    """Starts DCT Connection between two Android devices."""
    self.test_failure_reason = (
        nc_constants.SingleTestFailureReason.TARGET_START_ADVERTISING)
    # Start advertising.
    self.start_advertising(timeout=timeouts.connection_init_timeout)
  ...
  def transfer_file(
    ...
    try:
      ...
      self.discoverer.log.info(
          f'sending {num_files} payloads with type: {payload_type.name} and'
  ...
  def _transfer_file(
    ...
    asserts.assert_is_not_none(
        self._advertiser_payload_callback,
        'No DCT connection is set up, advertiser payload cb is none.',
    )
    asserts.assert_is_not_none(
        self._discoverer_payload_callback,
        'No DCT connection is set up, discoverer payload cb is none.',
    )
