...
"""Test MCC with 5G D2D medium and 2G STA with non-DBS capable devices.
...
failed, other mediums will be tried. As DBS is not supported by both devices,
and the STA is connected with 2G channel, D2D medium is using a 5G channel,
...
Test steps:
  ...
  4. Set up a connection and upgrade to any available Wi-Fi medium.
      * The expected wifi medium is WFD, but other meidums will be tried if
        WFD is failed.
...
_FILE_TRANSFER_SIZE_KB = constants.TRANSFER_FILE_SIZE_100MB
...
_THROUGHPUT_LOW_TIP_RAW = (
    'Check with the wifi chip vendor for any firmware issue in MCC mode.'
)
_THROUGHPUT_LOW_TIP = (
...
class Mcc5gAllWifiNonDbs2gStaTest(
  ...
  """Test class for CUJ MCC with 5G D2D medium and 2G STA test."""
  ...
  def test_mcc_5g_all_wifi_non_dbs_2g_sta(self):
    """Test the performance for CUJ MCC with 5G D2D medium and 2G STA test."""
    ...
    # Test Step: Set up a prior Nearby Connection through Bluetooth medium.
    ...
    upgrade_medium_metric = self.get_current_iteration_metrics().get(
        'upgrade_medium'
    )
    upgraded_medium_name = (
        upgrade_medium_metric.value if upgrade_medium_metric else None
    )

    if upgraded_medium_name in [
        constants.NearbyConnectionMedium.WIFI_DIRECT.name,
        constants.NearbyConnectionMedium.WIFI_HOTSPOT.name,
    ]:
    ...
    if (
        upgraded_medium_name == constants.NearbyConnectionMedium.WIFI_LAN.name
