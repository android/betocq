...
"""Base class for Wifi AWARE R4 tests using V3 APIs."""
...
MAX_CONSECUTIVE_ERROR = constants.SCC_PERFORMANCE_TEST_MAX_CONSECUTIVE_ERROR
_PAYLOAD_REQUEST_SIZE_KB = constants.TRANSFER_FILE_SIZE_10KB
_PAYLOAD_FILES_RESPONSE_SIZE_KB = constants.TRANSFER_FILE_SIZE_200MB
_PAYLOAD_TRANSFER_TIMEOUT = constants.WIFI_200M_PAYLOAD_TRANSFER_TIMEOUT
...
class AwareStaV3TestBase(nc_performance_test_base.NcPerformanceTestBase):
  """Base class for Wifi AWARE R4 tests using V3 APIs."""

  test_runtime: constants.NcTestRuntime
  wifi_info: constants.WifiInfo
  d2d_type: constants.WifiD2DType
  throughput_low_tip: str
  ...
  def setup_class(self) -> None:
    ...
    self.setup_wifi_env(
        d2d_type=self.d2d_type, country_code=_COUNTRY_CODE
    )
    # Test configurations.
    self.wifi_info = constants.WifiInfo.from_test_parameters(
        d2d_type=self.d2d_type, params=self.test_parameters
    )
