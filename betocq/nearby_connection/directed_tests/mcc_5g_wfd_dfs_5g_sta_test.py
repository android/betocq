...
"""This test is to test the Wifi MCC with 5G WFD and DFS 5G STA.
...
5G channel, but the WFD will be started in another 5G channel.
...
Test steps:
  ...
  4. Set up a connection with Wi-Fi Direct as upgrade medium.
      * Wi-Fi Direct will be set up by Nearby Connection in a 5G channel that is
        different form the STA channel.
...
_THROUGHPUT_LOW_TIP = (
    ...
    ' 5G DFS channel. Check with the wifi chip vendor about the possible'
...
class Mcc5gWfdDfs5gStaTest(nc_performance_test_base.NcPerformanceTestBase):
