...
"""This test is to test the Wifi SCC with WFD and STA in an indoor channel.
...
In this case, the feature is enabled on both devices so the WFD and STA both are
operating in the same 5G indoor channel.
...
Test steps:
  ...
  3. Connect advertiser to a 5G Wi-Fi network with indoor channel 36 in JP.
...
_THROUGHPUT_LOW_TIP = (
    'This is 5G SCC indoor WFD test case. Check STA and WFD GO frequencies in'
    ' the target logs (dumpsys wifip2p) and ensure they have the same value. In'
    ' the configuration file, enable_sta_indoor_channel_for_peer_network is set'
    ' to true. Check if the target device does support WFD group owner in the'
...
class SccIndoor5gWfdStaTest(performance_test_base.PerformanceTestBase):
  """Test class for Wifi SCC with 5G indoor WFD and 5G STA."""
  ...
  def test_scc_indoor_5g_wfd_sta(self):
    ...
    discoverer_sta_op_done = setup_utils.remove_current_connected_wifi_network(
        self.discoverer
    )
    ...
    advertiser_sta_op_done = nc_utils.connect_ad_to_wifi_sta(
        self.advertiser,
    ...
    if discoverer_sta_op_done or advertiser_sta_op_done:
    ...
    test_result_utils.set_and_assert_p2p_frequency(
        ...
        additional_error_message=(
            'Check if enable_sta_indoor_channel_for_peer_network is really True'
